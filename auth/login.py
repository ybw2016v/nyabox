...
def create_user(args):
    ...
    if udog is None:
        ...
        udog = Udog(uid=id,mid=tes["id"],nid=tes["username"],name=namedog,uuid=str(uuidog),stime=datetime.now(),token=str(uuidog),avatar=tes["avatarUrl"],text=tes["description"])
        db_session.add(udog)
        db_session.commit()
    else:
